...
```
IntegrationEnvironment ──1:∗── HotelIntegration ──1:1── Hotel
   (shared CM account,            (resort_code,
...
`reference` is our human-facing code (e.g. `ATN-8F3K2P`), generated by us and stable.
`external_reservation_id` holds the channel manager's ID, returned synchronously.
`external_confirmation_number` holds OPERA's, which arrives later over OXI — see the
two-stage confirmation in `property-integration.md` §4. Both are stored verbatim and never
regenerated.
...
`id · name · integration_type · endpoint · chain_code · environment · credential_ref ·
enabled · circuit_state · last_error_at · last_error_summary`

**One row serves all three hotels.** They share a single channel-manager account reaching a
single multi-property OPERA installation, so endpoint, credentials, and connection health
belong here rather than being duplicated three times. Breaker state lives here too, because
an outage on this connection affects every property at once.

`credential_ref` is a pointer into the secret manager — **never the credential**.

### HotelIntegration
`id · hotel_id · environment_id · resort_code · capabilities (jsonb) · enabled ·
last_sync_at · sync_status · booking_mode · last_error_at`

What is genuinely per property: the **OPERA resort code**, the capability flags, and the
sync state. `resort_code` is the key that makes one connection serve three hotels.

Nothing in either table may be serialised into any API response reachable from the public
site.
...
Populated by ARI pushed from the channel manager. This table is doing more work than it
looks: it is what makes group-wide search fast, **and it is what keeps the site browsable
and searchable when the shared OPERA or the channel manager is down**. With one installation
behind all three properties, an outage would otherwise take the whole group offline. Reading
from a pushed snapshot instead of querying live means only new reservation creation is
affected.

A partial index on `(hotel_id, date)` where `available_count > 0` keeps search reads cheap.
